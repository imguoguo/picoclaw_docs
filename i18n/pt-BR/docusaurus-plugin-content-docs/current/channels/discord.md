...
- Crie um aplicativo → Bot → Adicionar Bot
...
| `token` | string | Token do Bot obtido do Discord Developer Portal |
| `proxy` | string | URL de proxy HTTP/SOCKS (opcional) |
| `allow_from` | array | Lista de IDs de usuários permitidos (vazio = permitir todos) |
| `group_trigger` | object | Configurações de acionamento em grupo (veja abaixo) |
| `reasoning_channel_id` | string | Direcionar a saída de raciocínio para um canal separado |

### 5. Convidar o Bot

- OAuth2 → Gerador de URL
- Escopos: `bot`
- Permissões do Bot: `Send Messages`, `Read Message History`
...
Controle como o bot responde nos canais do servidor (não afeta mensagens diretas — o bot sempre responde em mensagens diretas):
...
- Entre em um canal de voz, então digite `!vc join` em um canal de texto para fazer o bot entrar
...
O canal de voz requer que `voice.tts_model_name` esteja configurado no `config.json`. Consulte [Configuração de Modelos](../configuration/model-list.md) para detalhes.
